...
async function renderUserDashboard(user) {
  ...
  target.innerHTML = leads.length ? leads.map(lead => {
    ...
    const updatedAt = lead.stageUpdatedAt || lead.submittedAt;
    return `<article class="lead live-project"><div class="lead-head"><h2>${escapeHtml(lead.requestType)}</h2><span>${new Date(lead.submittedAt).toLocaleString()}</span></div><div class="lead-meta"><span>Status: ${escapeHtml(lead.status)}</span><span class="stage">Current stage: ${escapeHtml(lead.stage)}</span><span>${escapeHtml(lead.timeline)}</span></div><div aria-label="Project stage: ${escapeHtml(lead.stage)}" class="stage-tracker">${tracker}</div><p class="tracking-updated">Live status · Last updated ${new Date(updatedAt).toLocaleString()}</p><p class="lead-message">${escapeHtml(lead.message)}</p></article>`;
